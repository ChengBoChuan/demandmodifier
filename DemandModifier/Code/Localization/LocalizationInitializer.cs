            ...
            try
            {
                ...
                try
                {
                    ...
                    if (Directory.Exists(modsPath))
                    {
                        ...
                        log.Debug(string.Format("方法 0 成功: {0}", modDirectory));
                    }
                }
                catch (Exception ex0)
                {
                ...
                if (string.IsNullOrEmpty(modDirectory))
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(codeBasePath))
                        {
                            ...
                            log.Debug(string.Format("方法 1 成功: {0}", modDirectory));
                        }
                    }
                    catch (Exception ex1)
                    {
                ...
                if (string.IsNullOrEmpty(modDirectory))
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(location))
                        {
                            ...
                            log.Debug(string.Format("方法 2 成功: {0}", modDirectory));
                        }
                    }
                    catch (Exception ex2)
                    {
                ...
                if (!string.IsNullOrEmpty(modDirectory))
                {
                    ...
                    {
                        log.Warn(string.Format("識別的目錄不是 DemandModifier: {0}", modDirectory));
                        modDirectory = null;
                    }
                }
                ...
                foreach (string jsonFile in jsonFiles)
                {
                    ...
                    try
                    {
                        ...
                        if (GameManager.instance != null && GameManager.instance.localizationManager != null)
                        {
                            ...
                            log.Info(string.Format("✓ 已註冊語言: {0} ({1})", localeId, jsonFile));
                            successCount++;
                        }
                        else
                        {
                            log.Warn(string.Format("✗ 本地化管理器不可用: {0}", fileName));
                            failureCount++;
                        }
                    }
                    catch (Exception ex)
